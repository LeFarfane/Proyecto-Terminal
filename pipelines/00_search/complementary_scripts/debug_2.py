#!/usr/bin/env python3
"""
geo_ibd_curator.py (The Verbose True Pivot Edition)
...
Key Features:
    1. Verbose Logging: Prints all NCBI commands, stderr, and stdout previews.
    2. The True Pivot: Uses direct text-search on the BioProject database.
...
- SRA Metadata Fetched: {args.fetch_runinfo}
...
def main() -> int:
    try:
        ...
        ap.add_argument("--out", default="geo_ibd_out", help="Master output folder")
        ap.add_argument("--fetch-runinfo", action="store_true",
                        help="Merge and fetch SRA/BioSample data via BioProject")
        ap.add_argument("--ignore-list", default="ignore_list.txt",
                        help="Text file containing GSE IDs to permanently skip")
        args = ap.parse_args()

        print("\n" + "=" * 50)
        print("👋 Welcome to the GEO Data Curator Pipeline (Verbose Edition)")
        print("=" * 50)
        ...
        ignore_filepath = Path(args.ignore_list).resolve()
        ...
        for i, e in enumerate(entries_to_process, 1):
            ...
            if args.fetch_runinfo:
                # Using the exact GSE string to execute the direct text-search pivot
                ...
                if rich_meta.get("available") and rich_meta.get("merged_csv_data"):
                    ...
                    print(f"\n⚠️  No raw SRA runs found for {e['GSE_ID']}. Skipping folder creation.")
